...
class Chat extends React.Component {
  ...
  rowRenderer = ({
    ...
    if (msg.is_info) {
      content = <div className="message-info">
        {msg.text}
      </div>
    } else {
      const imageStyle = {
      ...
      let image = <div className="letter-icon" style={imageStyle}>{msg.from_first_name[0]}</div>;
      if (msg.from_profile_image != null) {
        image = <img className="image-icon" src={"dc://" + msg.from_profile_image} alt="avatar"/>;
      }
      content = (
  ...
  render() {
    ...
    let rowCount = selectedChatMsgs != null ? Object.keys(selectedChatMsgs).length : 0
...
const mapStateToProps = (state) => {
  ...
  return {
    ...
    selectedChat: account && account.selected_chat && account.chats[account.selected_chat],
